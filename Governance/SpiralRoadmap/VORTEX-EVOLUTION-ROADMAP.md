...
| S1 | 🌡️ 市场脉搏 | MSS 闭环 | CP-01, CP-02 | 3.8d | 📋 未开始 |
| S2 | 📡 信号生成 | 信号生成闭环（含集成） | CP-03, CP-04, CP-10, CP-05 | 6.1d | 📋 未开始 |
| S3 | 🔬 回测闭环 | 回测 + 分析闭环 | CP-10, CP-06, CP-09 | 4.2d | 📋 未开始 |
...
> 核心算法集中在 S1-S2（三三制信号全链路 + 前移实证）；基础设施分布在 S0/S3-S5（数据/回测/交易/展示）；外挂均匀分布并承担早期可视与对照责任，全局占比约 27%。
...
├── L1/L2 数据集
        ▲               │
        └──── S2r ◄─────┘  (S3 回测质量门禁失败时触发微循环)
```

> `S2r` = S2 repair 子圈，仅允许对 IRS/PAS/Validation/Integration 做定向微修，不改 S0/S1 契约。
...
- **test**: `pytest tests/contracts/test_irs_* tests/contracts/test_pas_* tests/contracts/test_validation_* tests/contracts/test_integration_* tests/contracts/test_signal_probe_* -q`
- **artifact**: `irs_industry_daily` + `stock_pas_daily` + `validation_gate_decision` + `validation_prescription` + `integrated_recommendation` + `s2_cross_section_probe`
...
| S3-T5 轻量可视化 + 质量门评估 | 外挂 ENH-06 | `quicklook_equity_curve` + `quality_gate_report` | 在进入 S4 前形成可视反馈与 Go/No-Go 判定 | 0.3d |
...
4. A/B/C 对照表可生成且归档
5. 产出 `quicklook_equity_curve` 与 `quality_gate_report`
6. 若质量门为 FAIL，必须创建 S2r 回修任务并形成消费结论后方可继续

### 8.5.1 质量门阈值与 S2r 入口/出口（新增）
...
4. 在 `review.md` 写明“是否可进入 S4”的消费结论。

> 单一问题最多允许 2 次 S2r 循环；连续 2 次仍 FAIL 时，阻断主线并升级到路线图评审。

### 8.6 退出五件套

- **run**: `eq backtest --engine vectorized --start {start} --end {end}` + `eq backtest-quicklook --start {start} --end {end}`
- **test**: `pytest tests/contracts/test_backtest_* tests/contracts/test_analysis_* tests/contracts/test_backtest_quality_gate.py -q`
- **artifact**: `backtest_results` + `performance_metrics` + A/B/C 报表 + `quicklook_equity_curve` + `quality_gate_report`
